...
def polymorphic(_=None, /, *, on: Optional[str] = None):
    def deco(cls: _T) -> _T:
        ...
        if _FIELDS not in cls.__dict__:
            cls = dataclass(cls)
        annotations: dict[str, Any] = _get_annotations(cls)
        if not annotations:
            raise TypeError(
        ...
        if on is None:
            ...
            if on not in annotations:
                raise TypeError(
                    f"discriminator field '{on}' not found.")
            if on in _get_discriminator_names(cls):
                raise TypeError(
                    f"discriminator field '{on}' is already used.")
            discriminator = on
        field = None
        for f in fields(cls):
        ...
        if field.default is not MISSING or field.default_factory is not MISSING:
            raise TypeError(
                "the discriminator field cannot have a default value.")
...
def _init_subclass_(cls: type):
    ...
    for val in values:
        if not isinstance(val, mark.field.type):
            raise TypeError(
                f"{mark.field.type!r} required, but {val!r} is given")
